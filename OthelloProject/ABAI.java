...
public class ABAI implements IOthelloAI{
	...
	public Position decideMove(GameState gameState) {
        /*for (Position pos : gameState.legalMoves()) {
        ...
        for(int y = 0; y < boardValues.length; y++) {
            for(int x = 0; x < boardValues.length; x++) {
                System.out.print(boardValues[y][x]+" ");
            }
            System.out.println();
        }
    ...
    */
    ...
    private UtilityMove MAX_VALUE(GameState gameState) {
        ...
        for(Position move : gameState.legalMoves()) {;
            //if(newGM.insertToken(move)) {
                UtilityMove um2 = new UtilityMove(move); //MIN_VALUE(gameState);
                System.out.println("MAX Position row: "+move.row+", col: "+move.col+", utility = "+um2.getUtility());
                if (um2.getUtility() > v) {
                    v = um2.getUtility();
    ...
    private UtilityMove MIN_VALUE(GameState gameState) {
        ...
        for(var move : gameState.legalMoves()) { 
            ...
            //if(newGM.insertToken(move)) {
                UtilityMove um2 = MAX_VALUE(newGM);
                if(um2.getUtility() < v) {
                    v = um2.getUtility();
                    bestMove = um2.getMove();
                }
